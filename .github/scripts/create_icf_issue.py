#!/usr/bin/env python3
"""Create or reuse an issue requesting ICF overrides updates."""
...
def read_provisioning_template(
    workspace: Path, provisioning_path: str
) -> list[str]:
    try:
        content = (workspace / provisioning_path).read_text(encoding="utf-8")
    except FileNotFoundError:
        log(
            "::warning::No se encontró la plantilla de provisioning "
            f"en {provisioning_path}"
        )
        return []
    except OSError as exc:
...
def extract_properties_section(lines: list[str], source_path: str) -> str:
    ...
    if not lines:
        return (
            f"> **Nota:** No se encontró `{label}`. Carga la plantilla antes de "
            "continuar."
        )
    ...
    if not relevant:
        return (
            f"> **Nota:** No se detectó contenido utilizable en `{label}`. "
...
def github_request(
    ...
    try:
        ...
        raise RuntimeError(
            "GitHub API "
            f"{method} {endpoint} failed: {http_err.status} {http_err.reason} – "
            f"{detail}"
        ) from http_err
...
def main() -> int:
    ...
    if not secrets_lines:
        secrets_lines.append(
            "- _(no se identificó entorno de destino para actualizar)_"
        )
    secrets_section = "\n".join(secrets_lines)
    ...
    overrides_qa_b64 = os.environ.get(
        "ICF_OVERRIDES_QA_JSON_B64", ""
    ).strip()
    overrides_prod_b64 = os.environ.get(
    ...
    if content_b64:
        try:
            ...
            log(
                "::warning::No se pudo decodificar ICF_TEMPLATE_CONTENT_B64: "
                f"{exc}"
            )
    ...
    if not template_lines and override_path:
        ...
        try:
            ...
            log(
                "::warning::No se encontró la plantilla exportada en "
                f"{candidate}"
            )
        except OSError as exc:
            log(
                "::warning::No se pudo leer la plantilla exportada "
                f"{candidate}: {exc}"
            )
    ...
    if overrides_b64:
        try:
            ...
            log(
                "::warning::No se pudo decodificar ICF_OVERRIDES_JSON_B64: "
                f"{exc}"
            )
    ...
    def decode_override(custom_b64: str, fallback: str) -> str:
        ...
        try:
            ...
            log(
                "::warning::No se pudo decodificar overrides específicos "
                f"({custom_b64[:8]}…): {exc}"
            )
            return fallback
    ...
    replacements = {
        ...
        "{{ARTIFACT_DIR}}": os.environ.get(
            "ARTIFACT_DIR", "(sin directorio publicado)"
        ),
    ...
    if template_body:
        ...
        body = render_body(default_body, replacements)
